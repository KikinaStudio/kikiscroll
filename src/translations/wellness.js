const wellness = {
  fr: {
    // Overlay
    overlay_tagline: "Au croisement de la science, du son et du soin.",
    overlay_cta: "Entrer dans l'expérience sonore",

    // Intro section
    intro_eyebrow: "Composer la présence.",
    intro_start: "Lancer l'expérience sonore",
    intro_scroll: "Faites défiler lentement pour découvrir",

    // Full-screen intro screen (avant le scroll)
    intro_line_1: "Ceci n'est pas un site web.",
    intro_line_2: "C'est une expérience d'une minute.",
    intro_line_3: "Faites défiler lentement, avec le son.",

    // Section titles
    s0_title: "Le son, signature du soin.",
    s1_title: "La science de la sculpture acoustique.",
    s2_title: "Une signature, plusieurs espaces.",
    s3_title: "Composer l'état, pas l'ambiance.",
    s4_title: "Le son à l'écoute du geste.",
    s5_title: "Une composition par soin.",

    // Section 0 paragraphs — guide le lecteur dès l'intro à prêter l'oreille, en posant la page
    // elle-même comme démonstration : le son change avec chaque section, comme il changera dans son espace.
    s0_p1: "Tendez l'oreille. Ici, la musique évolue à mesure que vous avancez. C'est un avant-goût de ce que nous composons pour votre espace : un son qui épouse le lieu, l'instant et le geste.",
    s0_p2: "Chaque spa possède son ambiance sonore. Elle agit sur le système nerveux avant même le premier soin. C'est souvent la première chose que le client ressent, et l'une des dernières qu'il emporte avec lui.",
    s0_p3: "Hôtels, instituts, thermes, retraites de longévité : nous composons cette signature espace par espace, de l'accueil aux derniers instants de repos. Une musique vivante, écrite par des compositeurs et éclairée par les neurosciences.",

    // Section 1 paragraphs — piste nue continue + chaque espace s'accorde avec elle
    s1_p1: "Sous tous les espaces du spa court une même trame sonore, continue et discrète. Présente partout, elle ancre l'expérience et porte l'identité du lieu.",
    s1_p2: "Autour de cette trame, chaque espace reçoit sa propre musique. Réception, hammam, cabine, salon de repos : chaque pièce s'accorde sur le même socle harmonique.",
    s1_p3: "Le client passe d'un espace à l'autre sans jamais ressentir de rupture. La musique change de texture, jamais d'identité. Une seule respiration, plutôt qu'une suite de morceaux.",

    // Section 2 paragraphs
    s2_p1: "Le client oublie l'essentiel de ce qu'il entend. Mais il se souvient de ce qu'il a ressenti.",
    s2_p2: "De l'accueil au repos, un spa est une suite de moments, chacun avec son rôle et son intensité. Nous composons une signature sonore qui traverse tous les espaces, se transforme dans chacun d'eux, et reste en mémoire bien après le dernier soin.",
    s2_p3: "Le client ne retient pas un morceau. Il retient un lieu.",

    // Section 3 paragraphs
    s3_p1: "Un soin poursuit rarement une seule intention. Relâchement du corps, apaisement du système nerveux, présence attentive : ces états se dosent avec autant de précision qu'un protocole.",
    s3_p2: "À chacun de ces états répond une trame sonore qui lui est propre.",
    s3_p3: "Nos compositions glissent de l'un à l'autre au fil du soin, sans transition perceptible.",
    s3_p4: "Le praticien guide le geste. La musique guide le système nerveux. Les deux avancent ensemble, sans jamais avoir à se concerter.",

    // Section 4 paragraphs — la musique suit le geste en temps réel (capteur spatial)
    s4_p1: "En cabine, la musique n'est pas un décor. Elle accompagne le geste du praticien, en temps réel.",
    s4_p2: "Un capteur discret perçoit la pièce : la position du praticien, le rythme de ses gestes, la respiration du corps sur la table. La musique s'ajuste à ce qui se passe vraiment. Le praticien n'est plus accompagné d'une playlist, il est porté par une composition qui l'écoute.",
    s4_p3: "Faites l'essai. Activez votre caméra et bougez lentement devant elle. La musique s'ouvre avec votre mouvement, puis revient à la trame de fond dès que vous vous immobilisez. En cabine, ce sont des capteurs de mouvement qui jouent ce rôle : ils suivent le geste, jamais le visage. Une réponse vivante, propre à chaque soin, sans intrusion.",

    // Section 5 paragraphs — la partition épouse l'arc du soin, le capteur ajoute la couche live.
    s5_p1: "Au-delà de cette écoute en temps réel, chaque soin possède sa propre partition. Une heure de soin suit un arc : l'arrivée, la prise de contact, la profondeur, le relâchement, le retour.",
    s5_p2: "La musique peut ignorer cet arc, ou l'épouser. Nous composons avec lui : cinq strates sonores, une pour chaque phase, qui apparaissent et s'effacent à mesure que le corps se laisse aller.",
    s5_p3: "La composition suit le soin d'elle-même. Le capteur, lorsqu'il est présent, y ajoute la réponse au geste du praticien. Le corps se détend, et le système nerveux en garde la mémoire.",

    // Scroll prompt
    scroll_prompt: "Continuez à faire défiler lentement",

    // Section 1: Isolation toggle
    isolation_on: "Bruit ambiant atténué",
    isolation_off: "Bruit ambiant présent",

    // Section 5: phases du soin (5 strates = 5 phases d'un soin exemple)
    phase_label: "phase du soin",
    phases_title: "Phases du soin",
    phase_1: "Accueil",
    phase_2: "Contact",
    phase_3: "Profondeur",
    phase_4: "Relâchement",
    phase_5: "Retour",

    // Section 2: Zones (3 clés existantes réutilisées avec valeurs spa, + 4e clé recuperation)
    zone_entree: "Accueil et transitions",
    zone_entree_sub: "Le seuil",
    zone_entree_body: "Réception, vestiaires, couloirs, tisanerie. L'espace le plus animé du spa, là où l'agitation de la ville est encore présente. Notre composition adoucit les bruits parasites sans s'imposer, et prépare le système nerveux au premier soin.",
    zone_rayon: "Rituels de chaleur",
    zone_rayon_sub: "L'enveloppe",
    zone_rayon_body: "Hammam, sauna, douches sensorielles, parcours Kneipp. Une acoustique exigeante : surfaces dures, vapeur, résonance. Notre composition s'y installe avec des basses chaudes et des textures organiques. L'objectif n'est pas d'endormir, mais d'offrir une détente éveillée, qui accompagne la circulation et le souffle.",
    zone_equipe: "Cabines de soin",
    zone_equipe_sub: "Le geste",
    zone_equipe_body: "Modelage, drainage, soin du visage, soin énergétique. La main du praticien dialogue avec un système nerveux à fleur de peau. Nos trames sonores soutiennent le geste, sans à-coup ni surprise. Le client se laisse aller, le praticien travaille en confiance.",
    zone_recuperation: "Espaces de récupération",
    zone_recuperation_sub: "L'empreinte",
    zone_recuperation_body: "Salles de repos, tisanerie, espaces de calme. L'étape la plus sous-estimée du parcours, et l'une des plus précieuses. Nos compositions prolongent l'apaisement et étendent le bénéfice du soin bien au-delà de la porte de la cabine. Le souvenir d'un spa est rarement visuel. Il est sonore.",

    // Section 3: Neuro labels
    neuro_relaxation: "Relâchement du corps",
    neuro_regulation: "Apaisement nerveux",
    neuro_focus: "Présence attentive",

    // Section 4: Webcam — détection de mouvement, pas d'émotion. Le label "lecture
    // du geste" sert d'indicateur passif pendant que la musique suit le geste.
    webcam_active: "Caméra active",
    webcam_authorize: "Autoriser la caméra",
    webcam_reading: "Lecture du mouvement",
    webcam_hint: "Votre image ne s'affiche pas à l'écran et rien n'est enregistré : la caméra sert uniquement à percevoir le mouvement. Placez-vous face à elle, en pleine lumière, puis bougez lentement. La musique s'ouvre avec le geste et revient au calme dès que vous vous immobilisez.",
    webcam_move_hint: "Bougez la main devant la caméra : la barre suit votre mouvement.",
    webcam_denied: "Caméra bloquée. Autorisez l'accès dans les réglages du navigateur, puis réessayez.",
    webcam_unavailable: "Caméra indisponible. Vérifiez qu'aucune autre application ne l'utilise, puis réessayez.",

    // Footer
    footer_about: "Qui sommes-nous",
    footer_legal: "Mentions légales",
    footer_contact: "Nous contacter",
    footer_email_placeholder: "Votre email",
    footer_message_placeholder: "Votre message",
    footer_send: "Envoyer",
    footer_copyright: "© 2026 Kikina Lab. Tous droits réservés.",
    footer_mailto_subject: "Contact depuis Kikiscroll Wellness",
    footer_mailto_body_prefix: "De : ",

    // Wellness CTA intro (placée juste au-dessus du formulaire de contact)
    cta_title: "Dites-nous votre intention. On la compose.",
    cta_body: "Vous avez pensé chaque détail de l'expérience. Le son mérite le même soin. Écrivez-nous, on le crée sur mesure pour votre lieu.",
    cta_invite: "Écrivez-nous",
    cta_email: "bianca@kikinastudio.com",

    // Legal modal
    legal_title: "Mentions Légales",
    legal_editor: "Éditeur du site :",
    legal_editor_value: "Kikina Lab / Kikina Studio",
    legal_address_label: "Siège social :",
    legal_address_value: "Paris, France",
    legal_contact_label: "Contact :",
    legal_hosting_label: "Hébergement :",
    legal_hosting_value: "Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723",
    legal_privacy: "Ce site utilise une détection de mouvement par caméra implémentée en JavaScript natif (différence d'images sur canvas). Aucune donnée d'image n'est enregistrée ou transmise à des serveurs tiers ; l'analyse est effectuée localement dans votre navigateur en temps réel.",
    legal_close: "Fermer",
  },

  en: {
    // Overlay
    overlay_tagline: "At the crossroads of science, sound, and care.",
    overlay_cta: "Enter the sound experience",

    // Intro section
    intro_eyebrow: "Composing presence.",
    intro_start: "Start the sound experience",
    intro_scroll: "Scroll slowly to explore",

    // Full-screen intro screen (before the scroll)
    intro_line_1: "This is not a website.",
    intro_line_2: "It's a one-minute experience.",
    intro_line_3: "Scroll slowly. Sound on.",

    // Section titles
    s0_title: "Sound, as a signature of care.",
    s1_title: "Sound that cancels the room.",
    s2_title: "One signature, many spaces.",
    s3_title: "Compose the state, not the ambience.",
    s4_title: "Sound that listens to the gesture.",
    s5_title: "A composition for every treatment.",

    // Section 0 paragraphs — opens by guiding the user to listen, framing the page itself
    // as a small demonstration of context-aware sound (each section = a different climate).
    s0_p1: "Listen closely. On this page, the music shifts with every section you scroll into, a quiet demonstration of how, in your future space, sound will live in dialogue with the room, the moment, the gesture.",
    s0_p2: "Every spa carries a sonic climate that shapes the nervous system before the first treatment. It is often the first sensation a guest receives, and one of the last impressions they carry away.",
    s0_p3: "Hotels, day spas, thermal baths, longevity retreats: we compose that signature, space by space, from the threshold of reception to the final moments of recovery. Living music, written by composers, guided by neuroscience.",

    // Section 1 paragraphs — a continuous naked track + each space scored to harmonize with it
    s1_p1: "Our system listens to the room in real time and adapts the music to mask ambient noise.",
    s1_p2: "The result feels like noise-cancelling headphones, except no one is wearing any.",
    s1_p3: "Pure immersion, nothing in the way.",

    // Section 2 paragraphs
    s2_p1: "Guests forget most of what they hear. But they remember how a place sounded.",
    s2_p2: "From reception to recovery, a spa is a sequence of moments, each with its own role and its own intensity. We compose a sonic signature that runs through every space, takes a different shape in each, and stays in memory long after the last treatment ends.",
    s2_p3: "The guest never recalls a track. They recall the place.",

    // Section 3 paragraphs
    s3_p1: "A treatment has more than one intention. Somatic release, parasympathetic regulation, attentional grounding: these physiological states are as precise as a dosage.",
    s3_p2: "Each register has its own sonic fabric.",
    s3_p3: "Our compositions slide from one state to the next as the treatment unfolds, without perceptible transition.",
    s3_p4: "The practitioner guides the gesture, the sonic signature guides the nervous system. Both converge without coordination.",

    // Section 4 paragraphs — music follows the hands in real time, via a spatial sensor
    s4_p1: "In the treatment room, music should not be a backdrop. It should follow the hands at work, in real time.",
    s4_p2: "A spatial sensor reads the room: the position of the practitioner, the rhythm of the gesture, the breath of the body on the table. The composition adjusts to what is actually unfolding. The practitioner is no longer accompanied by a playlist, but partnered by a composition that listens to her.",
    s4_p3: "Try it now. Turn on your camera and move slowly in front of it. The music opens up with your movement and settles back to the underlying layer the moment you hold still. In the treatment room, the same logic runs through spatial sensors that follow the gesture, not the face. A calibrated response, alive, individual, without intrusion.",

    // Section 5 paragraphs — composed score follows the treatment arc, sensor adds the live layer
    s5_p1: "Behind this real-time adaptation, every treatment has its own original score. A one-hour treatment has an arc: an arrival, a first contact, a depth, a release, a return.",
    s5_p2: "The music can ignore that arc, or follow it. We compose alongside it: five strata written for five phases, entering and stepping back as the body settles.",
    s5_p3: "The composition adapts to the treatment by itself. The sensor, when present, adds a live response to the practitioner's gesture. The body softens. The nervous system remembers the precision.",

    // Scroll prompt
    scroll_prompt: "Keep scrolling slowly",

    // Section 1: Isolation toggle
    isolation_on: "Situated listening active",
    isolation_off: "Ambient noise",

    // Section 5: treatment phases (5 strata = 5 phases of an example treatment)
    phase_label: "treatment phase",
    phases_title: "Treatment phases",
    phase_1: "Welcome",
    phase_2: "Opening",
    phase_3: "Depth",
    phase_4: "Release",
    phase_5: "Return",

    // Section 2: Zones (3 existing keys reused with spa values, + 4th recuperation key)
    zone_entree: "Receptions and transitions",
    zone_entree_sub: "The threshold",
    zone_entree_body: "Reception, changing rooms, corridors, tea lounge. The loudest zone of the spa, where the urban state still lingers. Our composition masks the parasitic sounds without imposing itself, and prepares the nervous system before the first treatment.",
    zone_rayon: "Heat rituals",
    zone_rayon_sub: "The envelope",
    zone_rayon_body: "Hammam, sauna, sensory showers, Kneipp path. Extreme acoustics, hard surfaces, steam. Our composition meets the volume with warm bass and organic textures. The aim is not sedation but alert relaxation, supporting circulation and breath.",
    zone_equipe: "Treatment rooms",
    zone_equipe_sub: "The gesture",
    zone_equipe_body: "Massage, drainage, facial, energy work. The practitioner's hand dialogues with a raw nervous system. Our sonic textures support the gesture without randomness or surprise. The guest surrenders, the practitioner works in confidence.",
    zone_recuperation: "Recovery spaces",
    zone_recuperation_sub: "The imprint",
    zone_recuperation_body: "Relaxation lounges, tisanerie, quiet rooms. The most underestimated stage of the journey, and one of the most powerful. Our compositions extend the parasympathetic state and carry the treatment well beyond the cabin door. The memory of a spa is rarely visual. It is sonic.",

    // Section 3: Neuro labels
    neuro_relaxation: "Somatic release",
    neuro_regulation: "Parasympathetic regulation",
    neuro_focus: "Attentional grounding",

    // Section 4: Webcam — motion detection, no emotion read-out. The "reading the
    // gesture" label is a passive cue while the music follows the movement.
    webcam_active: "Motion sensor active",
    webcam_authorize: "Allow motion sensor",
    webcam_reading: "Reading the gesture",
    webcam_hint: "Motion sensor only. No recording.",
    webcam_move_hint: "Move your hand in front of the camera — the bar follows your gesture.",
    webcam_denied: "Camera blocked. Allow access in your browser settings, then try again.",
    webcam_unavailable: "Camera unavailable. Make sure no other app is using it, then try again.",

    // Footer
    footer_about: "About us",
    footer_legal: "Legal notice",
    footer_contact: "Contact us",
    footer_email_placeholder: "Your email",
    footer_message_placeholder: "Your message",
    footer_send: "Send",
    footer_copyright: "© 2026 Kikina Lab. All rights reserved.",
    footer_mailto_subject: "Contact from Kikiscroll Wellness",
    footer_mailto_body_prefix: "From: ",

    // Wellness CTA intro (sits right above the contact form)
    cta_title: "Tell us your intention. We compose it.",
    cta_body: "You have thought through every detail of the experience. The sound deserves the same care. Write to us and we will create it, tailored to your space.",
    cta_invite: "Write to us",
    cta_email: "bianca@kikinastudio.com",

    // Legal modal
    legal_title: "Legal Notice",
    legal_editor: "Site publisher:",
    legal_editor_value: "Kikina Lab / Kikina Studio",
    legal_address_label: "Registered office:",
    legal_address_value: "Paris, France",
    legal_contact_label: "Contact:",
    legal_hosting_label: "Hosting:",
    legal_hosting_value: "Vercel Inc., 440 N Barranca Ave #4133, Covina, CA 91723",
    legal_privacy: "This site uses webcam motion detection implemented in pure JavaScript (frame-difference on a canvas). No image data is recorded or transmitted to third-party servers; analysis is performed locally in your browser in real time.",
    legal_close: "Close",
  },
};

export default wellness;
